...
- **Build cBot** — пројектно-базирана радионица на `/ai/build`: **направите нови cBot** (јединствено име + језик) или **улучше постојећи** који има изворни код, затим **разговарајте** са моделом на `/ai/build/{projectId}` да напишете и расчишћате његов код. **Сваки prompt и одговор модела су персистирани са временским печатима** и преживе навигацију/освежавање; изворни код модела је примењен на пројекат при сваком обрту. **Направите** и **Покрените** cBot са исте странице (или га отворите у пуном уреднику). Сваки пројекат се појављује на листи са његовим **временом последње промене** и контролама за преглед/брисање.
...
- Web ендпоинти под `/api/ai/*` (AI Build chat `build/{id}/prompt` + `build/{id}/messages`, generate-project, review, analyze-backtest, optimize-params, optimize-run, post-mortem, sentiment, vision, curate, …). Свака ендпоинт функције прихвата опционални `?modelId=<credential>` да покреће тај један позив на одабраном моделу. Плус **model discovery** (`/api/ai/models/probe`, `/api/ai/usable-models`) и **per-feature bindings** (`/api/ai/feature-bindings`, `/api/ai/my-feature-bindings`). cBot пројекти, градња и покретање поново користе builder ендпоинте (`/api/builder/projects…`).
