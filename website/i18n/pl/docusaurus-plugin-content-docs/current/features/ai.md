...
- Web endpoints pod `/api/ai/*` (czat Build `build/{id}/prompt` + `build/{id}/messages`, generate-project, review, analyze-backtest, optimize-params, optimize-run, post-mortem, sentiment, vision, curate, …). Każdy endpoint funkcji akceptuje opcjonalne `?modelId=<credential>` aby uruchomić to jedno wezwanie na wybranym modelu. Plus **odkrywanie modeli** (`/api/ai/models/probe`, `/api/ai/usable-models`) i **wiązania na funkcję** (`/api/ai/feature-bindings`, `/api/ai/my-feature-bindings`). Projekty cBot, build i run ponownie używają endpointów builderu (`/api/builder/projects…`).
