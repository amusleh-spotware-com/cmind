...
- `AllowAiModelManagement` (lalai `true`) — tetapkan `false` untuk menyembunyikan **pelayaran model**, **pemilih model setiap halaman**, dan **pengikatan model setiap ciri**. Kesemuanya boleh diselaras pemilik pada masa larian dari **Tetapan → Penempatan** (disarungkan hidup pada `IOptionsMonitor`) dan dikatalogkan dalam `WhiteLabelCatalog`.
...
- **Bina cBot** — bengkel berbasis projek di `/ai/build`: **cipta cBot baru** (nama unik + bahasa) atau **tingkatkan satu sedia ada** yang mempunyai sumber, kemudian **berbual** dengan model pada `/ai/build/{projectId}` untuk menulis dan memperhalusi kodnya. **Setiap prompt dan balasan model disimpan dengan cap masa** dan terselamat pelayaran/muat semula; sumber model digunakan pada projek pada setiap giliran. **Bina** dan **Jalankan** cBot dari halaman yang sama (atau buka dalam editor penuh). Setiap projek muncul dalam senarai dengan masa **perubahan terakhir**-nya dan kawalan pandang/padam.
...
- Titik akhir Web di bawah `/api/ai/*` (chat Bina cBot `build/{id}/prompt` + `build/{id}/messages`, generate-project, review, analyze-backtest, optimize-params, optimize-run, post-mortem, sentiment, vision, curate, …). Setiap titik akhir ciri menerima `?modelId=<credential>` pilihan untuk menjalankan panggilan satu itu pada model pilihan. Tambah **penemuan model** (`/api/ai/models/probe`, `/api/ai/usable-models`) dan **pengikatan setiap ciri** (`/api/ai/feature-bindings`, `/api/ai/my-feature-bindings`). Projek cBot, bina dan jalankan menggunakan semula titik akhir pembina (`/api/builder/projects…`).
