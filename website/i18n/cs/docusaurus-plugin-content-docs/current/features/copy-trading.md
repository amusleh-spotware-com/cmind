...
| Pending order limit / stop / stop-limit | Umístit odpovídající pending order, nesen stop-loss / take-profit hlavního |
| Úprava pending orderu | Upravit zrcadlený pending order na místě (včetně stop-loss / take-profit) |
...
Nově spuštěný profil stručně zobrazuje stav **Starting** (nikoli zelený *Running*), zatímco jeho hostitel načítá referenční data a spouští první resync — ještě nezrcadluje objednávky na cílech. Přepne na **Running** v okamžiku, kdy se první resync dokončí a engine může začít kopírovat. Stav Starting je pro ovládání řádků považován za běžící (Start zakázán, Zastavit a live-logs povoleny, Úprava/Smazání blokováno), takže se profil během zahřívání nemůže restartovat ani upravovat. Fáze zahřívání je sledována in-process na uzlu, který hostitel profil; profil hostovaný na jiné replice (nebo ten, který nelze hostovat — jeho zdrojový/cílový účty nejsou propojeny přes Open API) zobrazuje svůj běžný stav.
...
- **Kopírovat SL / Kopírovat TP** — zrcadlit stop-loss / take-profit hlavního, nebo spravovat ochranu nezávisle. Platí pro **obě** otevřené pozice **a** spočívající pending ordery — kopie limit/stop/stop-limit je umístěna a upravena se stop-loss/take-profit hlavního orderu (vyměněny pod **Obrácením**), takže ochrana je připojena v okamžiku vyplnění pending, nikoli pouze poté.
...
- **SL/TP precizní normalizace** (M6) — zkopírované stop-loss/take-profit ceny zaokrouhleny na **cílový** symbol číselné presnosti před úpravou (na pozicích **a** umístění/úpravě pending orderu), takže hlavní cena s jemnější presností (nebo cross-broker číselné nesoulad) nikdy neutopí server `INVALID_STOPLOSS_TAKEPROFIT`.
...
- **Cross-broker pending-fill race** — vedlejší pending vlastního vedlejšího může vyplnit (jeho cena se dosáhla) v malém okně před tím, než se zpracuje hlavní fill/cancel event. To zanechává vedlejší pozici označenou zdrojovým **order** id, kterou by kanonické close/SL-TP cesty (klíčované zdrojovým **position** id) mohly vynechat. Na hlavní **fill** se rané vyplnění vedlejšího vyřadí a nahradí jednou kanonicky označenou tržní kopií — takže se cíl skončí přesně **jednou** kopií, nikdy zdvojovanou pozicí; na hlavní **cancel** se zavře přímo (hlavní nikdy neučinil obchod). Oba působí okamžitě, nikoli pouze na dalším resyncu. Vedlejší-straně SL/TP hit, který zavře kopii, kterou hlavní stále drží, je řízeno zdrojem a znovu otevřeno na dalším sjednocení (engine zrcadlí **hlavní** eventy; nespotřebovává vedlejší-straně exekuce).
