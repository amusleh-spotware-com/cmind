...
- **Soubory modelu:** nasměrujte `ModelPath` na adresář obsahující model ONNX GenAI — `genai_config.json`, tokenizer a `.onnx` váhy. Build CPU **Phi-3.5-mini-instruct** funguje dobře (dodaný default), například:
