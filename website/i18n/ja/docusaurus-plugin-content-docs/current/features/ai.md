...
- **cBotを構築** — プレーンイングリッシュプロンプトから**生成 → 構築 → AI修復**自己修復ループ（`build-strategy`）で実行可能なcBotへ、`/ai/build`で。**生成されたソースコードはビルドが完了したときに表示され**（コピーボタン付き）、ビルドログとともに — 成功*および*失敗時 — つまり、AIが書いたもの全体を見ることができ、エラーだけではない。
...
- **Settings → AI**（`/settings/ai`、所有者のみ）— プロバイダリスト + **プロバイダ追加/編集ダイアログ**（kind、ベースURL、モデル、オプションのキー、機能トグル、「デフォルトに設定」）+ **接続テスト**ボタン。
