...
1. **Deterministic tests** (xUnit, no network) — copy math + engine logic. Fast, CI, no secrets. Cover mọi money-management mode, every filter/option, engine resilience.
...
tests/IntegrationTests/CopyLive/
  ...
  LiveTokenBootstrapTests.cs      — one-shot: decrypt tokens from app DB into token cache
...
`secrets/openapi-cids.local.json` — cID login creds to authorize (một hoặc nhiều):
...
Refresh token **never expires**, vì vậy after one-time onboarding live tests work indefinitely: each run exchanges each cID's refresh token for fresh access token (rotation) — no browser, no prompts.

## One-time onboarding (fully automated — no dev interaction beyond saving creds)

Onboarding drives real cTrader ID login in headless browser từ saved cID creds, captures OAuth callback on local HTTPS listener at app's registered redirect (`https://localhost:7080/openapi/callback`), exchanges code for tokens, loads account list, writes multi-cID token cache. Run once per machine (hoặc when adding cID):
...
Live tests trade **only demo accounts**: fixture filters token cache to accounts with `IsLive == false` và connects to demo gateway, vì vậy order không bao giờ land on live/funded account even if live account authorized. Every position a test opens closed in cleanup.
...
# Live copy tests against real demo accounts (needs the two secrets files)
...
Without secrets files live tests print skip reason + pass as no-ops, vì vậy suite safe to run anywhere.
...
scale **up** và **down** for balance/leverage/capacity mismatch (the "golden rule") · lot-step
...
Open mirrors a market order (side / volume / label) · **reverse** flips side và **swaps SL/TP** ·
...
Reaches Connected after app auth · dropped connection reconnects và re-auths · fatal auth error faults ·
exponential backoff.
...
| CopySkipped | 1049 | a copy was skipped và why (slippage / direction / symbol_filter / size_zero / …) |
...
Logs emitted as Serilog compact JSON (structured props: `ProfileId`, `DestinationCtid`, `SourcePositionId`, `Symbol`, `Side`, `Volume`, …), shipped to OTLP when `OTEL_EXPORTER_OTLP_ENDPOINT` set. **Fully configurable** per category via standard config — ví dụ raise/lower copy-engine verbosity without touching code:
...
Host mirrors hơn market open/close. Each behaviour = per-destination opt-in flag on `CopyDestination` (`MirrorPartialClose` default on, `MirrorScaleIn`/`CopyPendingOrders`/`CopyTrailingStop` default off), guarded by intention methods, jsonb-persisted (migration `CopyAdvancedMirroringAndNodeAffinity`).
...
- If run refreshes but **cannot persist** new token (e.g. read-only mount), cached token dead, next run fails `ACCESS_DENIED`. Regenerate với headless onboarding:
  `CMIND_ONBOARD=1 dotnet test tests/E2ETests --filter FullyQualifiedName~OnboardingTests`.
- `LiveCopySecrets.SaveTokens` swallows write failures vì vậy read-only cache doesn't crash run, nhưng **live** in-cluster suite still needs **writable** cache (K8s Job copies Secret into emptyDir — xem deployment doc).

## Running the suite in a Kubernetes cluster

Whole suite runs in-cluster against Helm-deployed app, vì vậy regression caught in-cluster same as locally. Xem [`docs/deployment/kubernetes.md`](../deployment/kubernetes.md#in-cluster-test-suite).
...
- **`LiveCopyMatrix`** — `[Theory]`/`[MemberData]` option matrix: one real master open per row against demo accounts, each với differently-configured destination, asserting golden outcome. Rows: `one_to_one`, `half_multiplier`, `reverse` (opposite side), `manage_only` (opens nothing), `trading_hours_closed` (window excludes now → no copy), `source_label_block` (label filter → no copy), `lot_sanity_block` (ceiling → no copy).
- **`LiveCopyChaos`** — copy engine against hostile start: master already holds position before host starts, vì vậy convergence can only come from start-up resync (`Sync-Open-on-Start`), plus negative case với toggle off. Deterministic socket-flap / token-rotation / rejection chaos covered exhaustively by DST suite; this asserts live resync path.
...
## Running live + cluster suites yourself (NO excuses — everything is self-serviceable)

Everything below available in this environment. Do **not** claim "creds unavailable", "token stale", or "kind/helm not installed" — set them up và run them.

### 1. Get a fresh Open API token (headless OAuth, no browser interaction)

cTrader refresh tokens single-use, vì vậy cached token WILL go stale. Re-mint nó yourself from saved cID username/password (`secrets/openapi-cids.local.json` + `secrets/openapi-test-app.local.json`, or unified `secrets/dev-credentials.local.json`). Onboarding test drives **headless Edge** OAuth via Playwright, writes `secrets/openapi-tokens.local.json`:
...
If app run + cID linked in-app, extract app's latest refresh token straight from `app-pg-data` Postgres volume instead of re-authorizing — xem `LiveTokenBootstrapTests`, set `CMIND_VOLUME_CONN`.
...
Xem [../deployment/kubernetes.md](../deployment/kubernetes.md).
