...
| Ordem pendente limit / stop / stop-limit | Coloca a ordem pendente correspondente, carregando o stop-loss / take-profit do master |
| Alteração de ordem pendente | Altera a ordem pendente espelhada em lugar (incluindo seu stop-loss / take-profit) |
...
Um perfil recém-iniciado brevemente mostra um status **Starting** (não um *Running* verde) enquanto seu host carrega dados de referência e executa o primeiro resync — ele ainda não está espelhando ordens entre os destinos. Muda para **Running** no momento em que esse primeiro resync completa e o mecanismo pode copiar. Starting é tratado como running para os controles de linha (Iniciar desabilitado, Parar e logs ao vivo habilitados, Editar/Deletar bloqueados), assim um perfil em aquecimento não pode ser reiniciado ou editado durante inicialização. A fase de aquecimento é rastreada in-process no nó que hospeda o perfil; um perfil hospedado em outra réplica (ou um que não pode ser hospedado — suas contas de origem/destino não estão vinculadas via Open API) mostra seu status simples.
...
- **Copiar SL / Copiar TP** — espelha stop-loss / take-profit do master, ou gerencia proteção independentemente. Aplica a **ambas** posições abertas **e** ordens pendentes descansando — uma cópia limit/stop/stop-limit é colocada e alterada com o SL/TP da ordem do master (trocado sob **Reverso**), assim a proteção é anexada no momento o pendente preenche, não apenas depois.
...
- **Normalização de precisão SL/TP** (M6) — stop-loss/take-profit copiados preços arredondados para precisão de dígito do símbolo **destino** antes de alterar (em posições **e** colocação/alteração de ordem-pendente), assim preço do master com precisão mais fina (ou incompatibilidade de dígito entre brokers) nunca dispara `INVALID_STOPLOSS_TAKEPROFIT` do servidor.
...
- **Corrida de preenchimento de pendente entre brokers** — um pendente próprio do slave pode preencher (seu preço foi atingido) na pequena janela antes do evento de preenchimento/cancelamento do master ser processado. Isso deixa uma posição slave rotulada pelo **id da ordem** de origem, que os caminhos canônicos de fechamento/SL-TP (com chave por **id da posição** de origem) poderiam perder. Em um **preenchimento** do master o preenchimento antecipado do slave é aposentado e substituído por uma cópia de mercado canonicamente rotulada — assim o destino termina com exatamente **uma** cópia, nunca uma posição duplicada; em um **cancelamento** do master ela é fechada por completo (o master nunca fez o trade). Ambas atuam imediatamente, não apenas no próximo resync. Um preenchimento de SL/TP do lado slave que fecha uma cópia que o master ainda mantém é dirigido pela origem e reabre na próxima reconciliação (o mecanismo espelha **eventos do master**; ele não consome execuções do lado destino).
