---
description: "All three services (Web, MCP, CtraderCliNode) log via Serilog as compact JSON on stdout — container runtimes và log collectors (Loki, ELK, CloudWatch…"
---

# Logging & observability

All three services (Web, MCP, CtraderCliNode) log via **Serilog** as **compact JSON on stdout** —
container runtimes và log collectors (Loki, ELK, CloudWatch, Azure Log Analytics, Datadog) ingest
...
- **Sink 1 — Console (compact JSON).** `RenderedCompactJsonFormatter`; every event carries full
  ...
  cộng `trace_id` / `span_id` from ambient `Activity` (`ActivityEnricher`) và `LogContext` scopes.
  Trace ids let CloudWatch Logs Insights và Azure Log Analytics pivot log↔trace **even với no
  collector deployed**.
- **Sink 2 — OTLP (optional).** When `OTEL_EXPORTER_OTLP_ENDPOINT` set, logs also export over OTLP
  với same resource attributes, alongside OpenTelemetry **metrics** và **traces** (ASP.NET Core,
...
- **Sink 3 — Azure Monitor (optional).** When `APPLICATIONINSIGHTS_CONNECTION_STRING` set, traces
  và metrics export **natively** to Application Insights (`AddAzureMonitorTraceExporter` /
  `AddAzureMonitorMetricExporter`) — no collector. Xem cloud-native section below.
- **Source-generated messages.** App logs use strongly-typed `LogMessages` extensions
  (`Core/Logging/LogMessages.cs`) với stable `EventId`s — never ad-hoc `ILogger.LogInformation`.
...
Override at runtime với env vars, ví dụ `Serilog__MinimumLevel__Default=Debug`.
...
From collector, fan out to backend (Tempo/Jaeger traces, Prometheus metrics, Loki logs) với
...
connection string to Web và MCP as `APPLICATIONINSIGHTS_CONNECTION_STRING`. Result:
...
- **Logs** (compact JSON on stdout) land in **same Log Analytics workspace** via Container Apps
  `appLogsConfiguration`, vì vậy `AppTraces` / `ContainerAppConsoleLogs_CL` join on trace id.
...
- **Logs** stay on `awslogs` driver as compact JSON; CloudWatch Logs Insights auto-discovers JSON
  fields, vì vậy you `filter` / `stats` on `trace_id`, `service.name`, `@l`, etc.
...
Mapped in **all** environments (previously Dev-only) vì vậy Kubernetes và cloud probes work in
production.
