...
- **OpenAI 호환 엔드포인트** — 로컬 모델(Ollama, LM Studio, vLLM, llama.cpp `server`, LocalAI) 및 OpenAI 호환 클라우드(**Kimi / Moonshot** at `https://api.moonshot.ai/v1/`, OpenRouter, Groq, Together, Mistral, DeepSeek) 포함 — 모두 하나의 OpenAI 호환 어댑터를 통해, 기본 URL + 모델 + 키만 다릅니다. 공급자 추가 대화상자는 기본 URL + 샘플 모델을 채우는 일회성 **사전 설정**(Kimi, OpenAI, OpenRouter, Groq, DeepSeek, Mistral, Ollama, LM Studio)을 제공합니다.
