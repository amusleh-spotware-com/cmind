...
> **Статус.** P0–P4 су имплементирани и испоручени. Доменско језгро, перзистенција (EF `calendar` шема,
> append-only читање/писање, FRED + BLS + распоред централних банака, ingestion worker са праћењем
> свежине по извору), верзионирани JWT REST API, мобилно оријентисани `/economic-calendar` UI, MCP
> алати, cBot JWT API, упозорења за догађаје високог утицаја, пауза copy-trade news-blackout,
> overlay догађаја бектеста, SSE ток, HMAC-потписани webhookови и типизирани `CmindCalendarClient`
> су сви имплементирани и интеграционо тестирани. P5 екстра функције (аналитика изненађења, iCal/CSV
> извоз, претрага кључних речи, прикључиви консензус) су преостале ставке — погледајте фазе увођења
> испод.
