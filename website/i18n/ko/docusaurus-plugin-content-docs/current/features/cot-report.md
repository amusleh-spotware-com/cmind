...
각 차트에는 **줌 인/아웃**(및 리셋) 도구 모음 버튼이 있으며 시간 축을 드래그하여 줌할 수 있습니다. **Export CSV**는 선택한 시장 및 보고서 유형의 전체 주간 이력을 스프레드시트 준비 파일로 다운로드합니다. **Compare markets**를 사용하여 여러 시장을 단일 차트에 오버레이합니다 — 비교 차트는 각 선택 시장의 투기꾼 순 포지션과 COT 지수를 나란히 표시하므로 한 눈에 크로스마켓 포지셔닝을 읽을 수 있습니다.

## How the data flows

데이터베이스가 캐시입니다. 주간 수집 워커는 추적된 시장의 6개 CFTC 데이터셋을 가져오고 시장 목록을 업데이트한 후 각 새 보고서를 **멱등적으로** 추가합니다(재실행은 스냅샷을 복제하지 않음). 또한 데이터는 **요청 시 로드됩니다**: 시장이 처음 요청될 때 CFTC 소스에서 가져와 저장되며, 후속 모든 요청은 데이터베이스에서 직접 제공됩니다. 캐시는 **새 주간 보고서가 릴리스될 때 새로고침됩니다** — 최신 저장된 보고서가 1주 이상 오래되면 다음 요청은 투명하게 최신 데이터를 가져와 추가합니다(소스가 과도하게 요청되지 않도록 제한됨). 초기 로드는 수년간의 이력을 채웁니다; 소스 중단 시 최적의 캐시 데이터 제공으로 저하됩니다. 모든 것은 키 없이 기본 제공됩니다; 선택적 Socrata 앱 토큰은 레이트 제한만 높입니다.
