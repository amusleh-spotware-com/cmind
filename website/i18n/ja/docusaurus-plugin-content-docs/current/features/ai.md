...
- `AllowAiTasks`（デフォルト`true`）— `false`に設定すると**バックグラウンドAIタスク**機能を削除
  （`/ai/tasks`ページとタスクAPIは404を返す; ランナーは要求を停止）; 同期AIの特徴は引き続き動作します。
- `AllowAiModelManagement`（デフォルト`true`）— `false`に設定すると**モデル参照**および**機能ごとのモデルバインディング**を非表示にします。
  両方は所有者が**Settings → Deployment**からランタイム時にチューナブルであり（`IOptionsMonitor`上でライブにオーバーレイ）、`WhiteLabelCatalog`にカタログ化されています。
...
- **バックグラウンドAIタスク** — 長時間実行のAIジョブ（例えばcBotをビルドする）を選択のモデルで開始してから、ページを離れて結果に戻ります。複数のモデルを選択して比較します — 各自が自身のタスク（`/ai/tasks`）として実行されます。ウェブホストワーカーが自己修復リース上のタスクを要求し（ノードが死亡した場合、回収）、進捗をタスク別のアクティビティログにストリーミング。
- **モデルを参照して選択、機能ごと** — プロバイダエンドポイント（LM Studio / Ollama / vLLM / llama.cppの`GET /v1/models`、または組み込みカタログ）がアドバタイズするモデルを参照します。IDを手入力する代わりに、**各AI機能を別のモデルにバインド**して、複数のモデルが異なる機能を一度に提供します（バインドされていない機能はスコープのアクティブプロバイダーにフォールバック）。
...
- **マーケットアラート** — プロバイダがサポートしている場合、ウェブ検索グリundedの
...
- `/api/ai/*`下のWebエンドポイント（build-strategy、generate-project、review、analyze-backtest、
  optimize-params、optimize-run、post-mortem、sentiment、vision、curate、…）、**バックグラウンドタスク**（`/api/ai/tasks` create/list/detail/cancel/delete）、**モデル発見**（`/api/ai/models/probe`、`/api/ai/usable-models`）および**機能ごとのバインディング**（`/api/ai/feature-bindings`、`/api/ai/my-feature-bindings`）。
...
- **AI** ナビグループ — 機能ごとのBlazor**ページ**：cBotを構築（`/ai/build`）、
  ...
  チューンアドバイザー（`/ai/tune`）、最適化（`/ai/optimize`）、**AIタスク**（`/ai/tasks`）、+
