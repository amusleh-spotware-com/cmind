...
- **Settings → AI** (`/settings/ai`, owner-only) — список провайдеров с диалогом **Add / edit provider** (kind, base URL с per-kind hints включая Ollama/LM Studio localhost preset, модель, опциональный ключ, переключатели возможностей, "set as default") и кнопкой **Test connection**.
