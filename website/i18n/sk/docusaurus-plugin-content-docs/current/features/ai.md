...
- **Vytvorenie cBot** — projektom-založený workshop na `/ai/build`: **vytvorte nový cBot** (jedinečný názov + jazyk) alebo **vylepšite existujúci**, ktorý má zdrojový kód, potom **chatujte** s modelom na `/ai/build/{projectId}` na zápis a spresňovanie jeho kódu. **Každá výzva a odozva modelu je trvalo uložená s časovými pečiatkami** a prežijú navigáciu/opätovné načítanie; zdrojový kód modelu sa aplikuje na projekt v každom ťahu. **Vytvorte** a **Spustite** cBot z rovnakej stránky (alebo ho otvorte v plnom editore). Každý projekt sa objaví v zozname s jeho **časom poslednej zmeny** a ovládacími prvkami zobrazenia/odstránenia.
