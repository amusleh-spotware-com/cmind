...
(Microsoft.ML.OnnxRuntimeGenAI, เช่น Phi-3.5-mini) — ดังนั้นทุก deployment มี AI ที่ใช้งานได้
...
Phi-3.5-mini) ไม่ต้องการ **API key และไม่มีบริการภายนอก** และเมื่อ first startup — เมื่อไม่มี provider
...
- **โมเดล built-in หลายตัวสามารถอยู่ร่วมกันได้** แต่ละโมเดลที่ดาวน์โหลดอยู่ภายใต้ `ModelPath/<key>` โค้ดแคตตาล็อกที่ได้รับการคัดสรร (Phi-3.5-mini เป็นค่าเริ่มต้น บวก Phi-3-mini-128k) สามารถดาวน์โหลดและสลับจาก **Settings → AI** การเลือก built-in submodel จะโหลดมันในกระบวนการ ได้มา/bundle โมเดล: ดู [AI providers → built-in](../deployment/ai-providers.md#built-in-local-ai-onnx-shipped)
...
- `AllowAiTasks` (default `true`) — ตั้ง `false` เพื่อ **ลบฟีเจอร์งาน AI ในพื้นหลัง** (หน้า
  `/ai/tasks` และ task API คืน 404; runner หยุดการแจ้ง); AI features ที่ใช้งานพร้อมกันยังคงทำงาน
- `AllowAiModelManagement` (default `true`) — ตั้ง `false` เพื่อซ่อน **การเรียกดูโมเดล** และ **การผูกโมเดล
  ต่อฟีเจอร์** ทั้งคู่สามารถ owner-tunable ในรันไทม์จาก **Settings → Deployment** (overlaid live บน
  `IOptionsMonitor`) และระบุไว้ใน `WhiteLabelCatalog`
...
- **งาน AI ในพื้นหลัง** — เริ่มงาน AI ที่ใช้เวลานาน (เช่น build cBot) ด้วยโมเดลที่คุณเลือก จากนั้นออกจากหน้าและกลับมาเพื่อดูผลลัพธ์ เลือกหลายโมเดลเพื่อเปรียบเทียบ — แต่ละโมเดลจะทำงานเป็นงาน (`/ai/tasks`) แถบเว็บโฮสต์ claims งาน on a self-healing lease (reclaimed ถ้า node ตาย) และ streams progress เข้า per-task activity log
- **เรียกดูและเลือกโมเดล ต่อฟีเจอร์** — เรียกดูโมเดลที่ provider endpoint โฆษณา (`GET /v1/models` บน LM Studio / Ollama / vLLM / llama.cpp หรือ built-in catalog) แทนการพิมพ์ด้วยมือ id และ **ผูก AI feature แต่ละตัวไปยังโมเดลต่างกัน** ดังนั้นโมเดลหลายตัวให้บริการฟีเจอร์ต่างกัน ในเวลาเดียวกัน (ฟีเจอร์ที่ไม่ผูก fall back ไปที่ scope's active provider)
...
- Web endpoints ภายใต้ `/api/ai/*` (build-strategy, generate-project, review, analyze-backtest,
  optimize-params, optimize-run, post-mortem, sentiment, vision, curate, …) บวก **งานในพื้นหลัง** (`/api/ai/tasks` create/list/detail/cancel/delete) **การค้นหาโมเดล** (`/api/ai/models/probe`, `/api/ai/usable-models`) และ **การผูกต่อฟีเจอร์** (`/api/ai/feature-bindings`, `/api/ai/my-feature-bindings`)
...
- **AI** nav group — หนึ่ง Blazor **page ต่อ feature**: Build cBot (`/ai/build`), Review (`/ai/review`),
  ...
  Digest (`/ai/digest`), Tune Advisor (`/ai/tune`), Optimize (`/ai/optimize`) **AI Tasks** (`/ai/tasks`) บวก Portfolio Agent, Alerts,
