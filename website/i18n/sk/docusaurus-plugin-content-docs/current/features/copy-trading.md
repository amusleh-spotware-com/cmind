...
- **Zámka konfigurácie** (C9) — zámrznutie nastavení destinácie počas obdobia (`POST …/destinations/{id}/lock` s minútami). Pokiaľ je uzamknutá, destinácia nemôže byť odstránená (agregát odmieta s `CopyDestinationConfigLocked`) — úmyselný strážny prvek proti impulzívnym zmenám počas poklesu. Zámka vyprší automaticky na jeho časovú pečiatku.
