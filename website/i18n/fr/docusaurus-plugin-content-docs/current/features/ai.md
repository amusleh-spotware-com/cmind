...
- **Build cBot** — un atelier basé sur des projets sur `/ai/build` : **créer un nouveau cBot** (nom unique + langue) ou **améliorer un existant** qui a du code source, puis **dialoguer** avec un modèle sur `/ai/build/{projectId}` pour écrire et affiner son code. **Chaque prompt et réponse du modèle est persisté avec des horodatages** et survit à la navigation/rechargement ; la source du modèle est appliquée au projet à chaque tour. **Build** et **Run** le cBot depuis la même page (ou ouvrez-le dans l'éditeur complet). Chaque projet apparaît dans la liste avec son **heure de dernière modification** et des contrôles view/delete.
...
- Endpoints web sous `/api/ai/*` (Build chat `build/{id}/prompt` + `build/{id}/messages`, generate-project, review, analyze-backtest, optimize-params, optimize-run, post-mortem, sentiment, vision, curate, …). Chaque endpoint de fonctionnalité accepte un `?modelId=<credential>` optionnel pour exécuter cet appel sur un modèle choisi. Plus **découverte de modèles** (`/api/ai/models/probe`, `/api/ai/usable-models`) et **liaisons par fonctionnalité** (`/api/ai/feature-bindings`, `/api/ai/my-feature-bindings`). Les projets cBot, build et run réutilisent les endpoints builder (`/api/builder/projects…`).
