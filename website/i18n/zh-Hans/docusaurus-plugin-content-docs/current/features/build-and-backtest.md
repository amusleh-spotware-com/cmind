...
每个实例行（及其详情页）都有与状态相符的控件。**活动**实例显示**停止**；**终止**实例（已停止 / 已完成 / 失败）显示**启动（▶）**，以相同的 cBot、账户、品种、时间周期、参数集和镜像重新启动它（运行以运行方式重启，回测以回测方式重启）。点击停止会显示"正在停止…"提示，并禁用该图标直到完成；新建的运行会立即出现在列表中——无需刷新页面。

控制台日志在**实例终止时会被持久化**——运行（停止时）和**回测**（完成时）皆是如此——因此最近一次运行的日志在详情页仍可查看，即使容器已消失，也能通过**下载日志**图标下载。
