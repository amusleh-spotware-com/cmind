...
- **Build cBot** — um workshop baseado em projetos em `/ai/build`: **crie um novo cBot** (nome unico + idioma) ou **melhore um existente** que tenha fonte, depois **converse** com um modelo em `/ai/build/{projectId}` para escrever e refinar seu codigo. **Cada prompt e resposta do modelo e persistida com timestamps** e sobrevive navegacao/reload; a fonte do modelo e aplicada ao projeto em cada turno. **Build** e **Run** o cBot a partir da mesma pagina (ou abra-o no editor completo). Cada projeto aparece na lista com seu **ultimo tempo de mudanca** e controles de visualizacao/delecao.
...
- Endpoints web em `/api/ai/*` (build da AI chat `build/{id}/prompt` + `build/{id}/messages`, generate-project, review, analyze-backtest, optimize-params, optimize-run, post-mortem, sentiment, vision, curate, …). Toda acao de funcionalidade AI aceita um `?modelId=<credential>` opcional para executar essa unica chamada em um modelo escolhido. Alem disso **descoberta de modelos** (`/api/ai/models/probe`, `/api/ai/usable-models`) e **vinculos por funcionalidade** (`/api/ai/feature-bindings`, `/api/ai/my-feature-bindings`). Os projetos de cBot, build e run reutilizam os endpoints do builder (`/api/builder/projects…`).
