...
## Ovládanie životného cyklu inštancie

Každý riadok inštancie (a jej stránka s detailmi) má ovládacie prvky zodpovedajúce stavu. **Aktívna** inštancia zobrazuje **Zastaviť**; **terminálna** (Zastavená / Dokončená / Zlyhala) zobrazuje **Spustiť (▶)** na opätovné spustenie s rovnakým cBotom, účtom, symbolom, časovým rámcom, súpravou parametrov a image (beh sa spustí ako beh, backtest ako backtest). Kliknutie na Zastaviť zobrazí upozornenie „Zastavuje sa…" a deaktivuje ikonu, kým sa operácia nedokončí; novovytvorený beh sa v zozname objaví okamžite — bez opätovného načítania stránky.

Protokoly konzoly sa **pri ukončení inštancie uchovávajú** — pre beh (pri zastavení) aj pre **backtest** (pri dokončení) — takže protokoly posledného behu zostávajú viditeľné na stránke s detailmi a stiahnuteľné cez ikonu **Stiahnuť protokoly** aj po zmiznutí kontajnera.
