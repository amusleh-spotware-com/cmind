...
- **Tệp mô hình:** trỏ `ModelPath` vào một thư mục chứa mô hình ONNX GenAI — `genai_config.json`, trình mã hóa và trọng số `.onnx`. Bản dựng **Phi-3.5-mini-instruct** CPU hoạt động tốt, ví dụ:
...
- Khóa: [console.anthropic.com](https://console.anthropic.com/) → API keys.
- URL cơ bản: `https://api.anthropic.com/` · Mô hình: ví dụ `claude-opus-4-8`.
- Khả năng: tìm kiếm web + thị lực bật theo mặc định.

### OpenAI

- Khóa: [platform.openai.com/api-keys](https://platform.openai.com/api-keys).
...
- Khóa: [aistudio.google.com/app/apikey](https://aistudio.google.com/app/apikey).
