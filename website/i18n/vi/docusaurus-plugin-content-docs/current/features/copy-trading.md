...
**New Profile** mở một biểu mẫu **full-page** chuyên dụng (`/copy-trading/new`), không phải hộp thoại — bộ tùy chọn đủ lớn để một trang đọc tốt hơn trên điện thoại và máy tính để bàn. Nó thu thập mọi thứ phía trước: tên hồ sơ, nguồn (master) tài khoản, đích đến (slave) tài khoản (multi-select với nút **Select all**; master được chọn bị loại khỏi danh sách slave), + bộ tùy chọn từng đích đến đầy đủ. **Mỗi kiểm soát đều mang theo một mẹo trợ giúp** giải thích nó làm gì và cách sử dụng. Các đầu vào có cấu trúc sử dụng **các kiểm soát được xác thực đúng cách** — số/phần trăm thông qua các trường số, chế độ/hướng/bộ lọc thông qua chọn, bộ lọc ký hiệu thông qua danh sách thêm/xóa các chip ký hiệu, và bản đồ ký hiệu thông qua bảng thêm/xóa `Source → Destination (× multiplier)` — không bao giờ là một blob văn bản cách nhau bằng dấu phẩy. Tất cả các đầu vào được **xác thực trước khi lưu** — tên/nguồn/đích đến bị mất, tham số kích thước không dương, giới hạn lot âm/không nhất quán, phần trăm drawdown ngoài phạm vi, không có loại đơn hàng được bật hoặc bộ lọc ký hiệu trống sẽ xuất hiện dưới dạng danh sách lỗi + chặn lưu. Khi tạo, hồ sơ được tạo + mỗi slave được chọn được thêm vào với các cài đặt được chọn, sau đó trang quay trở lại danh sách Copy Trading.

**Import / export.** Toàn bộ khối cài đặt có thể được **xuất vào tệp JSON** và **nhập lại** để điền trước biểu mẫu, vì vậy một điều chỉnh có thể được sử dụng lại trên các hồ sơ mà không cần gõ lại. Bản đồ ký hiệu tương tự có thể được **xuất / nhập dưới dạng tệp CSV** (`Source,Destination,VolumeMultiplier`) — chuẩn bị một bản đồ ký hiệu broker lớn trong một bảng tính và tải nó trong một bước. Các kiểm soát ký hiệu và nhập/xuất CSV tương tự cũng có sẵn trong hộp thoại đích đến trên trang Copy Trading.

Hành động hàng tôn trọng vòng đời: **Start** chỉ được bật khi không chạy, **Stop** + **Pause** chỉ khi chạy, **Delete** bị vô hiệu hóa khi đang chạy + yêu cầu xác nhận trước khi xóa hồ sơ + đích đến.
