...
- **cBot Oluştur** — `/ai/build` adresindeki proje tabanlı bir çalıştay: **yeni bir cBot oluşturun** (benzersiz ad + dil) veya kaynağı olan **mevcut bir cBotu iyileştirin**, ardından `/ai/build/{projectId}` adresinde modelle **sohbet edin** ve kodunu yazın ve geliştirin. **Her prompt ve model yanıtı zaman damgası ile kalıcılaştırılır** ve gezinti/yeniden yükleme'de kalır; modelin kaynağı her turda projeye uygulanır. **cBotu aynı sayfadan Derleyin** ve **Çalıştırın** (veya tam editörde açın). Her proje listede **son değişiklik zamanı** ve görüntüle/sil kontrolleri ile görünür.
...
- `/api/ai/*` altında Web uç noktaları (AI Build chat `build/{id}/prompt` + `build/{id}/messages`, generate-project, review, analyze-backtest, optimize-params, optimize-run, post-mortem, sentiment, vision, curate, …). Her özellik uç noktası seçilen bir modelde o bir çağrıyı çalıştırmak için isteğe bağlı bir `?modelId=` kabul eder. Artı **model keşfi** (`/api/ai/models/probe`, `/api/ai/usable-models`) ve **özellik başına bağlamalar** (`/api/ai/feature-bindings`, `/api/ai/my-feature-bindings`). cBot projeleri, derleme ve çalıştırma, builder uç noktalarını yeniden kullanır (`/api/builder/projects…`).
