---
description: "Retail prop firms (FTMO-style) sell evaluation accounts: trader phải hit profit target trong khi ở trong risk limits (max daily loss, max…"
---

# Prop-firm challenge simulation

Retail prop firms (FTMO-style) sell **evaluation accounts**: trader phải hit profit target trong khi
ở trong risk limits (max daily loss, max total/trailing drawdown, consistency, time limits) trước
funded. cMind cho phép user tạo **custom challenge of any industry shape**, bind to
...
pass hoặc breach, ends challenge, marks it, alerts user.
...
- **`DailyLossLimit`** `(percent, basis)` — basis `Equity` (intraday, includes floating P&L) hoặc `Balance`
  (realized only).
- **`DrawdownLimit`** — `Static` (from starting balance), `TrailingPercent` (from peak equity), hoặc
  `TrailingThresholdDollar` (trails equity peak by fixed dollar amount, rồi **locks at starting
...
`InstantFunding`, hoặc `Custom` (full control). UI pre-fills template; user có thể adjust any field.

### Phases & status

- **Phases:** `Evaluation → Verification → Funded` (single-step skips Verification).
- **Status:** `Active`, `Passed`, `Failed`, cộng lifecycle `Stopped` (tracking paused) — `Create` starts
...
- **`RecordEquity(EquitySnapshot, now)`** — rolls trading day at day boundaries (captures previous
  day's profit for consistency rule), updates peak/daily peaks, rồi **fails on first breach**
  (daily loss → drawdown → time limit → inactivity, in order) hoặc advances phase when profit target,
  minimum-trading-day, consistency requirements all met. Out-of-order snapshots và records on
...
Tracking mirrors copy-trading hosting stack exactly; prop tracker = **read-only** cousin của
copy engine.

- **`PropFirmTrackingSupervisor`** (`src/Nodes/PropFirm`) — `BackgroundService` on each node, gated on
  `App:PropFirm:Enabled`. Mỗi cycle **claims** active challenges on self-healing lease
  (`AssignedNode` + `LeaseExpiresAt`; dead node's challenges reclaimed once lease lapses —
  same atomic `ExecuteUpdate` claim as copy trading, vì vậy two nodes never double-track), renews leases,
  pushes rotated tokens in place, stops hosts whose challenge left `Active`.
- **`PropFirmTrackingHost`** (`src/Nodes/PropFirm`) — một per challenge. Opens `IOpenApiTradingSession`
  for account và, on `App:PropFirm:EquityPollInterval`, recomputes live equity, feeds to
...
- **`PropFirmEquityCalculator`** (`src/CTraderOpenApi/Client`) — cTrader-faithful equity maths.
  Equity **not** delivered by Open API, vì vậy derived: `equity = balance + Σ(unrealized P&L)`,
  where each position's P&L là `priceDifference × units × quote→deposit rate + swap + commission`
  ...
  subscriptions. Pure và isolated — currency-conversion hot spot unit-tested on its own.
...
through structured alert/audit trail (`LogMessages`). Live UI reflects same status change. Đây
...
| GET | `/challenges/{id}` | một challenge |
| GET | `/templates` | industry presets cho create dialog |
...
UI: `/prop-firm` (nav *Prop Firm*, gated by `PropFirm` flag) lists challenges với **Start/Stop/Delete**
...
account live over Open API, feed equity automatically. Without it (default), domain và
...
- **Integration** — `IntegrationTests/`: `PropFirmChallengePersistenceTests` (round-trip + record-equity +
  soft-delete, enriched-rules + lease round-trip) và `PropFirmTrackingLeaseTests` (claim, contested lease,
