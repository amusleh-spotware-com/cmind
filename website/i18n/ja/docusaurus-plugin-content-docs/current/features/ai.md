...
- **cBotを構築** — プレーンイングリッシュプロンプトから**生成 → 構築 → AI修復**自己修復ループ（`build-strategy`）で実行可能なcBotへ、`/ai/build`で。**生成されたソースコードはビルドが完了したときに表示され**（コピーボタン付き）、ビルドログとともに（も同様にコピー可能） — 成功*および*失敗時。**失敗したビルドでさえ、cBotsに保存され**（実際の一意の名前で）、コンパイルエラーを修復して再ビルドできるよう「エディターで開く」リンクを提供し、作業を失うことはありません。
...
- `/api/ai/*`下のWebエンドポイント（build-strategy、generate-project、review、analyze-backtest、optimize-params、optimize-run、post-mortem、sentiment、vision、curate、…）。すべての機能エンドポイントは、選択したモデルでその1つの呼び出しを実行するためのオプション`?modelId=<credential>`を受け入れます。さらに**モデル発見**（`/api/ai/models/probe`、`/api/ai/usable-models`）および**機能ごとのバインディング**（`/api/ai/feature-bindings`、`/api/ai/my-feature-bindings`）。
